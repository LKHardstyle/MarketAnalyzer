    ...
    public static class Calculations
    {
        public static Dictionary<string, decimal> CalculateBrandsPriceChange(List<Product> products, SortType sortType)
        {
            ...
            foreach (var product in products) 
            {
                ...
                decimal priceChangePercentage = CalculateChangeInPercentage(firstPrice, lastPrice);
            ...
            var averagePriceChangesByBrand = brands.Select(b => new KeyValuePair<string,decimal>(b.Key, Math.Round(b.Value.Average(), 1))).ToList();
        ...
        {
            return ConvertToDecimal(product.Oct2022Price) ?? ConvertToDecimal(product.Sep2022Price) ??
                ConvertToDecimal(product.Jul2022Price) ?? ConvertToDecimal(product.May2022Price) ??
                ConvertToDecimal(product.Dec2021Price);
        }

        public static decimal? GetFirstNonNullPrice(Product product)
        {
            return ConvertToDecimal(product.Dec2021Price) ?? ConvertToDecimal(product.May2022Price) ?? 
                ConvertToDecimal(product.Jul2022Price) ?? ConvertToDecimal(product.Sep2022Price) ?? 
                ConvertToDecimal(product.Oct2022Price);
        }

        public static decimal CalculateChangeInPercentage(decimal? firstPrice, decimal? lastPrice)
        {
            //shouldnt divide by zero
        ...
        public static string CalculateChangeInPrice(List<Product> selectedProducts)
        {
            ...
            //Calculates the totalprices of the selected Products
            ...
            //Percentage Change
            decimal percentageChange = CalculateChangeInPercentage(totalFirstPrice, totalLastPrice);

            //Printing Message depending on percentageChange
